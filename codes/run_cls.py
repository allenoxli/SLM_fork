...
from _util import load_pickle, save_pickle, set_logger
...
def main(args):
    ...
    if args.do_unsupervised or args.do_supervised:
        ...
        if args.do_classifier:
            ...
            with torch.no_grad():
                for x_batch, seq_len_batch, uchars_batch, segments_batch, restore_orders in valid_dataloader:
                    segments_batch_infer = slm(x_batch.cuda(), seq_len_batch, mode='decode')
                    batch_labels = []
                    for input_ids, segment in zip(x_batch, segments_batch_infer):
            ...
            for step in range(args.cls_train_steps):
                ...
                if (step % args.save_every_steps == 0) or (step == args.cls_train_steps - 1):
                    ...
                    if args.do_valid:
                        ...
                        stdout, stderr = out.communicate()
                        stdout = stdout.decode("utf-8")
                        logging.info('Cls Validation results:\n%s' % stdout)
...
# all_real_labels = output['real_labels']
...
#     is_training=True,
#     batch_token_size=cls_batch_size
# )
...
# for i, (input_ids, segment) in enumerate(zip(all_input_ids, segments)):
#     label = torch.ones(input_ids.size(0))
#     real_label = torch.ones(input_ids.size(0))
#     label[segment] = 1
#     batch_labels.append(label)
#     batch_real_labels.append(real_label)
...
# for x_batch, seq_len_batch, uchars_batch, segments_batch, restore_orders in cls_dldr:
#     print()
#     break
# # %%

# ids = torch.tensor([   0, 1126,  133,  385,  542,    2,   15,  484,    1,  250,    2,   60,
#          699,    2,  584,  626,   73,    2,  103,   15,    1, 1237, 1314,   69,
#            1,    4,  390,  156,  955, 1126,    2,    5])

# a = [2, 2, 1, 4, 1, 0, 0, 2, 1, 0, 0, 3, 1, 1, 2, 4, 1, 2, 2, 1]
# label = torch.zeros(sum(a))
# idx = [sum(a[ :i]) for i in range(1, len(a))]
# # cum = 0
# # for i in a:
#     # cum += 1
# # idx = ]
...
# %%
# for i, (_input_ids, _pseudo_labels) in enumerate(zip(all_input_ids, all_labels)):
