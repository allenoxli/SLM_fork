...
if __name__ == '__main__':
    ...
    # parser.add_argument('--data', type=str, nargs="+", default='pku')
    # parser.add_argument('--device', type=str, default='0')
    # args = parser.parse_args()

    os.environ["CUDA_VISIBLE_DEVICES"] = '0'       ### only use two GPUs
    ...
    config_file = 'models/slm_4_config_bert.json'
    slm_config = model.SLMConfig.from_json_file(config_file)
    slm = model.SegmentalLM(
        config=slm_config,
        init_embedding=None,
        hug_name=hug_name,
        encoder_mask_type=None,
        do_masked_lm=False,
        dim_narrow=None
    )

    m_path = 'models_bert_impact_42_grad/unsupervised-msr-4/best-checkpoint'
    slm.load_state_dict(torch.load(m_path)['model_state_dict'])

    model = slm.context_encoder.encoder

    # model.resize_token_embeddings(len(tk))
    model.to(device)
    model.eval()

    upper_bound, lower_bound = 2, 6

    data_list = ['msr', 'pku', 'as', 'cityu']
    data_list = ['msr']
    print(data_list)
    for data in data_list:
        out_dir = f'./log/zz/bound{upper_bound}'
